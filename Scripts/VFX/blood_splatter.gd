...
func _ready() -> void:
	...
	modulate = fresh_color
	animation_finished.connect(_on_vacuum_finished)
...
func _spawn_sequence(generation: int) -> void:
	...
	if spawn_main_splat and not BloodPools.main_textures.is_empty():
		var main_tex: Texture2D = BloodPools.main_textures[rng.randi_range(0, BloodPools.main_textures.size() - 1)]
	...
	if not BloodPools.big_textures.is_empty():
		...
		var big_count := rng.randi_range(big_count_min, big_count_max)
		for i in range(big_count):
			var pos := big_center + _find_scatter_position(big_scatter_radius)
			var tex: Texture2D = BloodPools.big_textures[rng.randi_range(0, BloodPools.big_textures.size() - 1)]
	...
	if not BloodPools.small_textures.is_empty():
		...
		var small_count := rng.randi_range(small_count_min, small_count_max)
		for i in range(small_count):
			var pos := small_center + _find_scatter_position(small_scatter_radius)
			var tex: Texture2D = BloodPools.small_textures[rng.randi_range(0, BloodPools.small_textures.size() - 1)]
			pending.append(_make_pending(tex, pos, false))
...
func _find_scatter_position(radius: int) -> Vector2:
	...
	for attempt in range(max_attempts):
		...
		var dist := rng.randf_range(0, radius) * pixel_size
	...
	var fallback_angle := rng.randf_range(0, TAU)
...
func _instantiate_splat(splat: PendingSplat) -> void:
	...
	if splat.is_main:
		...
		if constrain_rotation_90:
			...
			spr.rotation = rng.randf_range(0, TAU)
		if allow_flip:
			spr.flip_h = rng.randi_range(0, 1) == 1
			spr.flip_v = rng.randi_range(0, 1) == 1

	add_child(spr)
	splat_sprites.append(spr)

func clear_splats() -> void:
	for sprite: Sprite2D in splat_sprites:
...
func _on_vacuum_finished() -> void:
	if animation != "BloodVacuum":
		return
	clear_splats()
	get_parent().hide()
