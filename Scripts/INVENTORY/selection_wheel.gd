...
func _draw() -> void:
	if not game_paused:
		var offset : Vector2 = SPRITE_SIZE * - 0.5
		...
		for i in (total_slots):
			var rads : float = TAU * i / (total_slots)
		...
		for i in range(0,items.size()):
			...
			var step : float = TAU / (total_slots)
			var start_rads : float = step * (i - 1)
			if start_rads < 0: start_rads = 0
			else: start_rads = step * (i)
			var end_rads : float  = start_rads + step
			var mid_rads : float  = (start_rads + end_rads) * 0.5 * - 1
			...
			if selection == i :
				...
				for j in (points_per_arc + 1):
					var angle : float  = start_rads + j * (step) / points_per_arc
...
func joystick_process() -> void :
	if self.visible:
		...
		var angle : float  = -fmod(dir.angle(), TAU)
		if angle <0:
			angle +=TAU
